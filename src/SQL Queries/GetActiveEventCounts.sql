...
CREATE  PROCEDURE [dbo].[GetActiveEventCounts]
	@EventId INT = NULL
AS
BEGIN
...
, fi.ImagePath
from (
Select EventDate.EventId, eventDate.EventDateId
...
where EventDate.IsActive = 1 and EventDate.DateOfEvent > getdate()and ev.EventID = IsNull(@EventID,ev.EventID)
) evts
